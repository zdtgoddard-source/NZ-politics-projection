...
# ===== ELECTORATE CATEGORY PROBABILITIES =====
ELECTORATE_PROBS = {
    "safe": 0.95,
    "likely": 0.75,
    "lean": 0.60,
...
# IMPORTANT: make party_names match the order you want in the sheet
party_names = ["Labour", "National", "Greens", "ACT", "NZ First", "TPM", "Opportunity"]

# ===== OTHERS AS REAL PARTY =====
OTHERS_MEAN = 2.0
OTHERS_STD = 1.5
...
# bloc seat distributions (for graph)
...
for _ in range(NUM_SIMULATIONS):
    ...
    # --- RECORD PARTY SEAT DISTRIBUTIONS ---
    for p in party_names:
...
# ===== SPACE-SEPARATED SEAT DISTRIBUTION TABLE (PARTIES + BLOCS) =====
...
for seat in range(min_seat, max_seat + 1):
    ...
    # Party seat distributions (in the same order as header)
    for p in party_names:
        row.append(seat_results[p].get(seat, 0))

    # Bloc seat distributions (distribution of total bloc seats)
    row.append(left_bloc_seat_results.get(seat, 0))
    row.append(right_bloc_seat_results.get(seat, 0))

    print(*row, sep=" ")

# ===== OVERHANG SEAT DISTRIBUTION TABLE =====
